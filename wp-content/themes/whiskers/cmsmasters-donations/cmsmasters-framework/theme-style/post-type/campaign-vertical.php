...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	if ($image) {
		echo '<div class="cmsmasters_campaign_wrap_img">';
			whiskers_thumb_rollover($camp->ID, 'cmsmasters-square-thumb', false, $link, false, false, false, false, false, false, true, false, false);
		echo '</div>';
	}
	?>
	<div class="cmsmasters_campaign_cont">
		<div class="cmsmasters_campaign_wrap_heading">
			<?php 
			if ($rest_amount) {
				whiskers_donations_campaign_rest_amount($camp->ID);
			}
			
			whiskers_donations_campaign_heading($camp->ID, 'h2', $link);
			?>
		</div>
		<?php
		if ($excerpt) {
		...
		if ($donation_but) {
			whiskers_donations_campaign_donate_button($camp->ID);
		}
		?>
	</div>
	<?php
	if ($donated_percent) {
		whiskers_donations_campaign_donated($camp->ID, 'page', 'vertical', true, true, $campaign_color);
	}
	?>
</article>
