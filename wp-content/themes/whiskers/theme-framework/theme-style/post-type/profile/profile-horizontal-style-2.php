...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_profile_horizontal ' . $columns_num . ' ' . $style); ?>>
	...
	<?php
	whiskers_get_profile_color(get_the_ID(), 'horizontal', 'style_2');
	
	echo '<div class="cmsmasters_profile_img_wrap' . (has_post_thumbnail() ? ' enable_thumbnail' : ' disable_thumbnail') . '">';
		
		if (has_post_thumbnail()) {
			whiskers_thumb(get_the_ID(), 'cmsmasters-square-thumb', true, false, false, false, false, true, false, false, false, 'cmsmasters_theme_icon_image', false);
		} else {
