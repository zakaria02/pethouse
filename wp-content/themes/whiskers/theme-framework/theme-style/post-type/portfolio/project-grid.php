...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && whiskers_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	<div class="project_outer">
		...
		<?php 
			whiskers_thumb_rollover(get_the_ID(), $project_thumb_size, false, false, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		?>
		</div>
		<?php
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				$excerpt ? whiskers_project_exc_cont() : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				whiskers_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
