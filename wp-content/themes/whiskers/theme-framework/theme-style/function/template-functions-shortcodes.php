...
function whiskers_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true, $link_target = false) { 
	if ($type == 'post') {
...
function whiskers_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function whiskers_slider_post_check_exc_cont($type = 'post') {
	$exc = whiskers_slider_post_exc_cont($type, false);
...
function whiskers_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function whiskers_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsmasters_slider_post_author">' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'whiskers') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
...
function whiskers_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function whiskers_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function whiskers_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = whiskers_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = whiskers_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function whiskers_slider_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_attr__('Read More', 'whiskers');
	}
