...
function whiskers_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function whiskers_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function whiskers_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['whiskers' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date">' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
				'</abbr>' . 
...
function whiskers_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'whiskers') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
		...
		if ($cmsmasters_option['whiskers' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'whiskers') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
					'<span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
				'</a>' . 
...
function whiskers_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['whiskers' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
					whiskers_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
				'</span>';
			}
...
function whiskers_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = whiskers_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['whiskers' . '_blog_post_tag']) {
			$out .= '<span class="cmsmasters_post_tags">' . 
				get_the_tag_list(esc_html__('Tags:', 'whiskers') . ' ', ', ', '') . 
			'</span>';
		}
...
function whiskers_post_exc_cont($length = 20, $show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
		wpautop(whiskers_excerpt($length, false)) . 
	'</div>' . "\n");
	
	
	if (whiskers_excerpt($length, false) != '') {
		if ($show) {
...
function whiskers_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['whiskers' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function whiskers_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = whiskers_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = whiskers_get_global_options();
			
			if ($cmsmasters_option['whiskers' . '_blog_post_comment']) {
				$out = whiskers_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function whiskers_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
	$out = '<figure>' . 
